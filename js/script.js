...
function addImageBurst(x, y, baseSize = 200) {
	...
	// Taille basée sur la plus petite dimension de l'écran pour être toujours visible
	const minScreen = Math.min(window.innerWidth, window.innerHeight);
	const isMobile = minScreen <= 768;
	// Sur mobile: au moins 25% de la plus petite dimension, garantit une bonne visibilité
	const adjustedBaseSize = isMobile
		? Math.max(baseSize, minScreen * 0.28)
		: baseSize;
	const size = adjustedBaseSize * (0.75 + Math.random() * 0.5);
	
	// Random: une partie tombe, une partie disparaît
	const willFall = Math.random() < 0.6;
...
function spawnWishMessage() {
	...
	// Position horizontale : toujours dans l'écran visible
	// Marge de sécurité pour les textes (environ 50% de la largeur max du texte)
	const isMobile = window.innerWidth <= 768;
	const textSafeMargin = isMobile ? 12 : 8; // % de marge de chaque côté
	const minX = textSafeMargin;
	const maxX = 100 - textSafeMargin;
...
function startWishesLoop() {
	...
	// Timing élégant — images une par une
	const isMobile = window.innerWidth <= 768;
	const intervalDelay = isMobile ? 1200 : 1400;
	const betweenDelay = isMobile ? 300 : 350;

	// Quelques messages au démarrage (pas d'images tout de suite)
	...
	setTimeout(() => {
		...
		setTimeout(() => {
			...
			setTimeout(() => {
				...
				setTimeout(() => {
					introCompleted = true;
					// D'abord: fade-out du texte intro
					introText.style.animation = 'none';
					...
					setTimeout(() => {
						...
						// Cacher l'overlay, puis lancer feux d'artifice + images
						setTimeout(() => {
							introOverlay.classList.add('hide');
							beginFireworks();
						}, 2500);
...
function showFinaleWarning() {
	...
	warn.innerHTML = "<div style='font-size:clamp(2rem,8vw,5rem);font-weight:900;color:#fff;" +
		"text-shadow:0 0 30px #ff69b4,0 0 60px #ff1493,0 0 100px #ff69b4;font-family:Georgia,serif;" +
		"letter-spacing:3px;'>✨ Finale ✨</div>" +
		"<div style='font-size:clamp(1rem,3vw,1.8rem);color:#ffb6c1;margin-top:8px;letter-spacing:2px;'>Pour toi Latifa 💗</div>";
...
// ============================================================
//  FINALE ÉPIQUE — se déclenche après 1 minute
// ============================================================
function launchEpicFinale() {
	// 1) Arrêter les vœux texte
	stopWishesLoop();

	// Vider les vœux existants
	if (appNodes.wishesLayer) {
		appNodes.wishesLayer.innerHTML = "";
	}

	// 2) Activer le mode finale
	isFinalePhase = true;
	imageBurstEnabled = false;
	store.setState({
		config: Object.assign({}, store.state.config, { finale: true, autoLaunch: true })
	});
	configDidUpdate();

	// 3) Lancer un barrage massif — phase 1 (0-8s) : rapide
	const finaleInterval1 = setInterval(() => {
		seqRandomFastShell();
		seqRandomFastShell();
		seqRandomFastShell();
	}, 120);

	// Phase 2 (8-15s) : encore plus dense
	setTimeout(() => {
		clearInterval(finaleInterval1);
		const finaleInterval2 = setInterval(() => {
			seqRandomFastShell();
			seqRandomFastShell();
			seqRandomFastShell();
			seqRandomFastShell();
...
// ============================================================
//  ÉCRAN FIN — photos en colonne + message final
// ============================================================
function showHeartGalleryEnd() {
	togglePause(true);
	...
	style.textContent =
		"@keyframes twinkleEnd{from{opacity:.2}to{opacity:1}}" +
		"@keyframes pulseGlowEnd{from{text-shadow:0 0 20px #ff69b4,0 0 40px #ff1493;transform:scale(1)}" +
	...
	starsDiv.style.cssText = "position:fixed;inset:0;pointer-events:none;overflow:hidden;z-index:0;";
	for (let i = 0; i < 60; i++) {
		const s = document.createElement("div");
		const sz = 1 + Math.random() * 2.5;
		s.style.cssText = "position:absolute;border-radius:50%;background:#fff;" +
			"left:" + (Math.random()*100) + "%;top:" + (Math.random()*100) + "%;" +
			"width:" + sz + "px;height:" + sz + "px;opacity:" + (0.3+Math.random()*0.7) + ";" +
			"animation:twinkleEnd " + (1.5+Math.random()*2) + "s " + (Math.random()*3) + "s infinite alternate;";
	...
	floatDiv.style.cssText = "position:fixed;inset:0;pointer-events:none;overflow:hidden;z-index:0;";
	const floatSymbols = ["💗","💖","✨","🌸","💝","⭐"];
	for (let i = 0; i < 18; i++) {
		const h = document.createElement("div");
		h.style.cssText = "position:absolute;bottom:-60px;left:" + (Math.random()*100) + "%;" +
			"font-size:" + (14+Math.random()*20) + "px;" +
			"animation:floatHeart " + (6+Math.random()*8) + "s " + (Math.random()*8) + "s infinite linear;";
		h.textContent = floatSymbols[Math.floor(Math.random()*floatSymbols.length)];
		floatDiv.appendChild(h);
	}
	overlay.appendChild(floatDiv);

	// Titre
	const titleWrap = document.createElement("div");
	titleWrap.style.cssText = "position:relative;z-index:1;text-align:center;margin-bottom:6px;animation:slideInEnd 0.8s 0.2s both;";
	const title = document.createElement("div");
	title.style.cssText = "font-size:clamp(1.4rem,5vw,2.6rem);font-weight:900;" +
		"background:linear-gradient(90deg,#ff69b4,#fff,#ff1493,#ffd700,#ff69b4);" +
		"background-size:300% 300%;-webkit-background-clip:text;-webkit-text-fill-color:transparent;" +
		"background-clip:text;animation:shimmerText 3s ease infinite;font-family:Georgia,serif;letter-spacing:2px;";
	title.textContent = "\uD83C\uDF82 Joyeux Anniversaire \uD83C\uDF82";
	titleWrap.appendChild(title);
	overlay.appendChild(titleWrap);

	const subtitle = document.createElement("div");
	subtitle.style.cssText = "color:#fff;font-size:clamp(2.5rem,8vw,5rem);font-weight:900;" +
		"text-align:center;margin-bottom:28px;letter-spacing:4px;" +
	...
	msg.style.cssText = "color:#ffe4f0;font-size:clamp(1rem,3vw,1.3rem);" +
		"text-align:center;max-width:400px;line-height:1.8;" +
		"text-shadow:0 0 15px rgba(255,105,180,0.8);" +
		"animation:slideInEnd 1s " + (0.4 + loadedImages.length*0.12 + 0.3) + "s both;" +
		"padding:0 16px;position:relative;z-index:1;" +
		"border-top:1px solid rgba(255,105,180,0.3);padding-top:20px;";
...
class Shell {
	...
	burst(x, y) {
		...
		if (imageBurstEnabled && !isFinalePhase && (!hasActiveWishes() || wishesStopped)) {
			// Fréquence uniforme: 30% de chance par explosion
			const willShowImage = Math.random() < 0.3;
			
			if (willShowImage) {
				// Toujours baser la taille sur la plus petite dimension visible
				const minScreen = Math.min(window.innerWidth, window.innerHeight);
				const isMobile = minScreen <= 768;
				const baseSize = isMobile
					? Math.max(minScreen * 0.30, this.spreadSize * 0.35)
					: Math.max(150, this.spreadSize * 0.4);
